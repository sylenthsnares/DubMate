...
; 4.5 DubMate: components page, so the optional ~2 GB Pack Builder AI pipeline can be
;     opted into during the flow instead of via a post-install message box.
...
; Closes a running DubMate before install. Without this, the previous version's
; uninstaller cannot delete the in-use executable, ExecWait reports failure, and
; the template surfaces a generic "unable to uninstall" error (synthetic code 2).
; /T also takes down the Python engine and cloudflared sidecars, which hold open
; handles inside the install directory.
!macro DubMateCloseRunningApp
  ...
  ${If} $R9 == 0
    ...
    ${Else}
      ...
      nsExec::ExecToStack 'taskkill /F /T /IM "${MAINBINARYNAME}.exe"'
      Pop $R9
      ; Give Windows a moment to release the file handles before touching $INSTDIR.
      Sleep 2000
    ${EndIf}
  ${EndIf}
!macroend
...
Function .onInit
  ...
  ; DubMate: a running instance locks its own .exe and the files under
  ; resources\, which makes the previous version's uninstaller fail.
  !insertmacro DubMateCloseRunningApp

  ; DubMate: pick the default state of the optional Pack Builder component.
  Call InitPackBuilderDefault
FunctionEnd
...
; DubMate: optional Pack Builder AI pipeline (PyTorch + Demucs + Whisper).
;
; The installer deliberately does NOT download the ~2 GB payload here: blocking the
...
; section above has been parsed, while Call resolves forward references at link time.
Function InitPackBuilderDefault
  ; An explicit /AI flag wins, so silent and passive installs can opt in.
  ;
  ; The switch is deliberately NOT called /PACKBUILDER: FileFunc's GetOptions matches
  ; an option as a plain prefix and never checks for a separator afterwards, so
  ; "/PACKBUILDER" would also satisfy this script's existing "/P" (passive) switch.
  ClearErrors
  ${GetOptions} $CMDLINE "/AI" $R9
  ${IfNot} ${Errors}
